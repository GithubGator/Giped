<?php
	...
	*/
	...
	/*
	//Pour générer le code de la fonction controleContrainte
	echo $code;
	exit();
	*/
	$wrapperName = "DONNEE";
		...
		if($xml->nodeType==XMLReader::ELEMENT && $xml->name == $wrapperName){
						...
						*/
						...
						case "ANSA2":
							...
							if($tabCol[$cle] == "1850" || $tabCol[$cle] == "1880"){
								echo $cptErreur++.": valeur '".$cle."' erron&eacute;e : ".$tabCol[$cle]." pour la  ligne ".$i."<br />";	
			...
			*/
			...
			foreach($listeVariables as $cle){
				...
				/*
				if(!isset($tabCol[$cle]))
					echo $cptErreur++.": valeur '".$cle."' non presente pour la  ligne ".$i."<br />";	
				*/
				// pour bdd
				$cols.=$cle.",";
				...
				if ($val != "NULL" && $val != ""){
					...
					$csvstr .= $separateur;
					//echo $cptErreur++.": valeur '".$cle."' manquante pour la  ligne ".$i."<br />";	
				}else{
					$csvstr .= $separateur;
					//echo $cptErreur++.": valeur '".$cle."' vide pour la  ligne ".$i."<br />";	
				}
				//if($trace)
					//echo $cle ."--->".$val."<br />";
				// on initialise
				
				controlCoherence($cle,$val);
	...
	*/
	...
	echo "</pre>";
	 ...
	 //Traitement du fichier créé dans le répertoire accessible par Web
	 $nomRepertoire = "../../chiffres/".$anneeRef;
	if (is_dir($nomRepertoire)) {
                   // echo 'Le répertoire existe déjà!';  
        }else { 
		mkdir($nomRepertoire);
		//echo "Création :".$nomRepertoire;
	}
	...
	echo "<br/>";
	if($test) {
		echo "Variables pertinentes :<br />";
		print_r($tabVariablesPertinentes);
		echo "<br/>";
		echo "<br/>";
		echo "Variables presentes :<br />";
		arsort($tabVariablesPresentes);
		foreach($tabVariablesPresentes as $cle=>$val){
			echo $cle." ".$tabVariablesPertinentes[$cle]."|".$val."<br />";
		}
...
function envoi_mail_chiffres ( $destinataire, $add = "", $messageType="RDD",$CD="00") {
	...
	switch($messageType){
		case "RDD" :
			...
			<a href='http://srvdecret/chiffres/index.php?id=".$add."' >Informations relatives à ce fichier</a><br />
	...
	switch ($cle){
		...
		case "ANAIS":
			...
			if( ($val < $tabCol['ANSA1'] && $tabCol['ANSA1']!= '9999')  ||  ($val < $tabCol['ANSA2'] && $tabCol['ANSA2']!= '9999')  || ($val < $tabCol['ANSMERE'] && $tabCol['ANSMERE']!= '9999') || ($val < $tabCol['ANSPERE'] && $tabCol['ANSPERE']!= '9999')  )
				echo $cptErreur++.": valeur '".$cle."'->". $val." antérieure aux adultes (".  $tabCol['ANSA1'] ." | ". $tabCol['ANSA2']." | ".  $tabCol['ANSMERE']." | ". $tabCol['ANSPERE'].")<br />";
		...
		case "DATDECAP":
			...
			if($val==""){
				if($tabCol['DECAP'] != ''){
					echo $cptErreur++.": valeur '".$cle."' manquante car DECAP est présent<br />";
				}
			}else{
				//echo $cptErreur++.": valeur '".$cle."' non attendue <br />";
				if($val !="9999-99-99")
		...
		case "DATDECMDPH":
			...
			if($val==""){
				if($tabCol['HANDICAP'] == '2'){
					echo $cptErreur++.": valeur '".$cle."' manquante car mineur pris en charge HANDICAP  <br />";
				}
			}else{
				//echo $cptErreur++.": valeur '".$cle."' non attendue <br />";
				if($val !="9999-99-99")
		...
		case "DATDECPE":
			...
			if($val==""){
				...
				//echo $cptErreur++.": valeur '".$cle."' non attendue <br />";
				//on mémorise la plus garnde date de décision comme variable $anneeConcernée
				/*
		...
		case "DATIP":
			...
			if($val==""){
				if($tabCol['CODEV'] == '1'){
					echo $cptErreur++.": valeur '".$cle."' manquante CODEV =".$tabCol['CODEV']."<br />";
				}
			}else{
				//echo $cptErreur++.": valeur '".$cle."' non attendue <br />";
				if($val !="9999-99-99")
