...
$nbLignes=$sansAUTRLIEUAR->num_rows;
if($nbLignes == 0){
	$texte = messageCle('AUTRLIEUAR');
	if($texte == ""){
		$PDF->TitreSectionVert($numParagraphe.utf8_decode("Placement judiciaire - autre lieu d'accueil régulier"));
		$texte= "Aucune anomalie constatée";
	}else{
		$PDF->TitreSectionOrange($numParagraphe.utf8_decode("Placement judiciaire - autre lieu d'accueil régulier"));
	}
...
if ($nbLignes == 0){
	...
	$PDF->Write(4,"\n".utf8_decode("Existence d'un autre lieu d'accueil régulier du mineur pour les décisions judiciaires de placement ou décisions relatives à l'autorité parentale (variable AUTRLIEUAR)."));
	...
	$PDF->Ln(4);
	$texte = messageEnLien('AUTRLIEUAR');
	if($texte != ""){
		$PDF->Write(4,"\n".utf8_decode($texte));
	}
	$PDF->SetFont('Arial','B',9);
