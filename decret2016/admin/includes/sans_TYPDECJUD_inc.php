...
$nbLignes=$sansTYPDECJUD->num_rows;
if($nbLignes == 0){
	$texte = messageCle('TYPDECJUD');
	if($texte == ""){
		$PDF->TitreSectionVert($numParagraphe.utf8_decode("MJIE ou AEMO - type d'intervention"));
		$texte= "Aucune anomalie constatée";
	}else{
		$PDF->TitreSectionOrange($numParagraphe.utf8_decode("MJIE ou AEMO - type d'intervention"));
	}

}else{
	$PDF->TitreSectionRouge($numParagraphe.utf8_decode("MJIE ou AEMO - type d'intervention"));
}
$posY_debut = $PDF->GetY();
...
if ($nbLignes == 0){
	...
	$PDF->Write(4,"\n".utf8_decode("Type d'intervention mise en oeuvre au titre de la décision judiciaire d'action éducative en milieu ouvert, ou d'investigation (variable TYPDECJUD)."));
...
$j=0;
while ($record = $sansTYPDECJUD->fetch_assoc()) {
