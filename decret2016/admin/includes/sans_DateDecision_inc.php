...
$nbLignes=$DateDecisionManquantes->num_rows;
if($nbLignes == 0){
	$PDF->TitreSectionVert($numParagraphe.utf8_decode('Date de décision non renseignée'));
}else{
	$PDF->TitreSectionRouge($numParagraphe.utf8_decode('Date de décision non renseignée'));
}
$posY_debut = $PDF->GetY();
...
if ($nbLignes == 0){
	...
	$PDF->Write(4,"\n".utf8_decode("Chaque enregistrement du fichier doit comporter la date de décision (DATDECPE)."));
...
$j=0;
while ($record = $DateDecisionManquantes->fetch_assoc()) {
