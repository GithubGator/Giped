...
$nbLignes=$DateDebutManquantes->num_rows;
if($nbLignes == 0){
	$PDF->TitreSectionVert($numParagraphe.utf8_decode('Date de début effective non renseignée'));
}else{
	$PDF->TitreSectionRouge($numParagraphe.utf8_decode('Date de début effective non renseignée'));
}
$posY_debut = $PDF->GetY();
...
if ($nbLignes == 0){
	...
	$PDF->Cell(80,8, utf8_decode("Chaque enregistrement du fichier doit comporter une date de début effective de la mesure/prestation (variable DATDEB)."));
	$PDF->Ln(6);
	...
	$PDF->SetFont('Arial','I',9);
	$PDF->Cell(80,8, utf8_decode("Chaque enregistrement du fichier devrait comporter la date de début effective de la mesure/prestation (variable DATDEB)."));
	$PDF->Ln(6);
