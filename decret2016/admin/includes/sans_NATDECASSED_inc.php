...
$nbLignes=$sansNATDECASSED->num_rows;
$nbPertinents = $NATDECASSED_nonNSP->num_rows;

if($nbLignes == 0){
	if($nbPertinents == 0){
		$PDF->TitreSectionOrange($numParagraphe.utf8_decode('Nature de la décision judiciaire absente'));
	}else{
		$PDF->TitreSectionVert($numParagraphe.utf8_decode('Nature de la décision judiciaire absente'));
	}
}else{
	$PDF->TitreSectionRouge($numParagraphe.utf8_decode('Nature de la décision judiciaire absente'));
}
$posY_debut = $PDF->GetY();
...
if ($nbLignes == 0){
	...
	$PDF->Cell(80,8, utf8_decode("Si décision judiciaire de protection de l'enfance, en préciser la nature (variable NATDECASSED)."));
	$PDF->Ln(6);
	...
	$PDF->Cell(80,8, utf8_decode("Si décision judiciaire de protection de l'enfance, en préciser la nature (variable NATDECASSED)."));
	$PDF->Ln(4);
	$texte = messageEnLien('NATDECASSED');
	if($texte != ""){
		$PDF->Write(4,"\n".utf8_decode($texte));
	}
	$PDF->SetFont('Arial','B',9);
...
$j=0;
while ($record = $sansNATDECASSED->fetch_assoc()) {
