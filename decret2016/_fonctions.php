...
//    die('req');
//    var_dump($connection);
//    $requete = " SELECT * FROM Giped2016.NOMENCLATURE ";
    //$requete = " SELECT NEWFIELD, Type,Nsp FROM NOMENCLATURE WHERE 1 ORDER BY NEWFIELD ";
...
//        die('else');
        echo "Erreur dans l'ex&eacute;cution de la requ&ecirc;te <br>'$requete'<br>";
...
function  import_request_variables_54($s='GP',$pre="e") { // fonction deprecie dans php 5.4 mais toujours pr�sente dans php 5.3
    extract($_REQUEST);
}


function logtmp($txt,$time=false){
    //return 0;
    $fp = fopen('/var/www/html/decret2016/logs/tmp'. date("Y-m-d").'.txt','a+');
    fseek($fp,SEEK_END);
    $nouvel=$txt."\r\n";
    if($time) $nouvel = date("d-m-Y H:i:s")." : ".$nouvel;
    fputs($fp,$nouvel);
    fclose($fp); //basta
}
